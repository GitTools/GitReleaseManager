    ...
    public class CreateCommand : ICommand<CreateSubOptions>
    {
        ...
        public async Task<int> Execute(CreateSubOptions options)
        {
            ...
            if (!string.IsNullOrEmpty(options.Milestone))
            {
                ...
                release = await _vcsService.CreateReleaseFromMilestoneAsync(options.RepositoryOwner, options.RepositoryName, options.Milestone, releaseName, options.TargetCommitish, options.AssetPaths, options.Prerelease).ConfigureAwait(false);
            }
            else
            {
